        ...
        // PaymentMethodNonceCreatedListener, 
{
    ...
    @Override
    public void onStart() {
        if (this.dropInClient == null) this.createDropInClient(null);
    }
    ...
    private void createDropInClientAtStartup() {
        try {
            ...
            DropInClient lDropInClient = new DropInClient(this.cordova.getActivity(), "sandbox_38wckxr8_kmtt9dpdddndmd2f"); // this.clientTokenProvider);
    ...
    /**
     * @param callbackContext The callback context to call when dropInClient is ready, or null if none.
     */
...
//        });
    ...
    //     try {
    //         this.sendToListener("error", new JSONObject().put("message", error.getMessage()));
    ...
    */
    ...
    private String parseVersionRequested(final JSONObject obj, final String field) {
        try {
            if (!obj.has(field))
    ...
    private Boolean parseBoolean(final JSONObject obj, final String field, final Boolean defaultValue) {
        try {
            if (!obj.has(field))
                return defaultValue;
    ...
    private int parseInt(final JSONObject obj, final String field, final int defaultValue) {
        try {
            if (!obj.has(field))
                return defaultValue;
    ...
    private ThreeDSecureAdditionalInformation parseThreeDSecureAdditionalInformation(
        ...
        if (input.has("shippingAddress")) {
            ret.setShippingAddress(parseThreeDSecurePostalAddress(input, "shippingAddress"));
        }
        if (input.has("shippingMethodIndicator")) {
          ret.setShippingMethodIndicator(input.getString("shippingMethodIndicator"));
        }
        ...
        if (input.has("accountCreateDate")) {
          ret.setAccountCreateDate(input.getString("accountCreateDate"));
        }
        if (input.has("accountChangeIndicator")) {
          ret.setAccountChangeIndicator(input.getString("accountChangeIndicator"));
        }
        ...
        if (input.has("accountPwdChangeIndicator")) {
          ret.setAccountPwdChangeIndicator(input.getString("accountPwdChangeIndicator"));
        }
        if (input.has("accountPwdChangeDate")) {
          ret.setAccountPwdChangeDate(input.getString("accountPwdChangeDate"));
        }
        if (input.has("shippingAddressUsageIndicator")) {
          ret.setShippingAddressUsageIndicator(input.getString("shippingAddressUsageIndicator"));
        }
        if (input.has("shippingAddressUsageDate")) {
          ret.setShippingAddressUsageDate(input.getString("shippingAddressUsageDate"));
        }
        if (input.has("transactionCountDay")) {
          ret.setTransactionCountDay(input.getString("transactionCountDay"));
        }
        if (input.has("transactionCountYear")) {
          ret.setTransactionCountYear(input.getString("transactionCountYear"));
        }
        if (input.has("addCardAttempts")) {
          ret.setAddCardAttempts(input.getString("addCardAttempts"));
        }
        if (input.has("accountPurchases")) {
          ret.setAccountPurchases(input.getString("accountPurchases"));
        }
        if (input.has("fraudActivity")) {
          ret.setFraudActivity(input.getString("fraudActivity"));
        }
        if (input.has("shippingNameIndicator")) {
          ret.setShippingNameIndicator(input.getString("shippingNameIndicator"));
        }
        if (input.has("paymentAccountIndicator")) {
          ret.setPaymentAccountIndicator(input.getString("paymentAccountIndicator"));
        }
        if (input.has("paymentAccountAge")) {
          ret.setPaymentAccountAge(input.getString("paymentAccountAge"));
        }
        if (input.has("addressMatch")) {
          ret.setAddressMatch(input.getString("addressMatch"));
        }
        ...
        if (input.has("orderDescription")) {
          ret.setOrderDescription(input.getString("orderDescription"));
        }
        if (input.has("taxAmount")) {
          ret.setTaxAmount(input.getString("taxAmount"));
        }
        if (input.has("userAgent")) {
          ret.setUserAgent(input.getString("userAgent"));
        }
        if (input.has("authenticationIndicator")) {
          ret.setAuthenticationIndicator(input.getString("authenticationIndicator"));
        }
    ...
    private ThreeDSecurePostalAddress parseThreeDSecurePostalAddress(final JSONObject obj, final String field) {
    ...
    private ThreeDSecureRequest parseThreeDSecureRequest(final JSONObject obj, final String field) throws JSONException {
        ...
        if (input.has("billingAddress")) {
            req.setBillingAddress(parseThreeDSecurePostalAddress(input, "billingAddress"));
        }
        ...
        if (input.has("additionalInformation")) {
            req.setAdditionalInformation(parseThreeDSecureAdditionalInformation(input, "additionalInformation"));
        }
    ...
    private synchronized void launchDropIn(
        ...
        // not available...
        // dropInRequest.setRequestThreeDSecureVerification(parseBoolean(request, "requestThreeDSecureVerification", false));
        // not available...
        // dropInRequest.setCollectDeviceData(parseBoolean(request, "collectDeviceData",false));
...
//        forceCreateDropInClient(dropInRequest);
    ...
    public void onDropInSuccess(DropInResult dropInResult) {
        ...
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.AMEX)
            paymentMethod = "AMEX";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.GOOGLE_PAY)
            paymentMethod = "GOOGLE_PAY";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.DINERS_CLUB)
            paymentMethod = "DINERS_CLUB";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.DISCOVER)
            paymentMethod = "DISCOVER";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.JCB)
            paymentMethod = "JCB";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.MAESTRO)
            paymentMethod = "MAESTRO";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.MASTERCARD)
            paymentMethod = "MASTERCARD";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.PAYPAL)
            paymentMethod = "PAYPAL";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.VISA)
            paymentMethod = "VISA";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.VENMO)
            paymentMethod = "VENMO";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.UNIONPAY)
            paymentMethod = "UNIONPAY";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.HIPER)
            paymentMethod = "HIPER";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.HIPERCARD)
            paymentMethod = "HIPERCARD";
        if (dropInResult.getPaymentMethodType() == DropInPaymentMethod.UNKNOWN)
    ...
    */
    ...
    public void sendToDropInContext(final JSONObject data) {
        ...
        // } catch (JSONException e) {
        //     Log.d(TAG, "sendToDropInContext() -> Failed: " + e.getMessage());
        //     if (this.dropInCallbackContext != null) {
        //         this.dropInCallbackContext.error(e.getMessage());
        //         this.dropInCallbackContext = null;
        //     }
        // }
    }
